...
static int
  display_create_image_screen(int const mode)
{
  ...
  do
    {
      ...
      _screen->len_x = info.mod_x_resolution;
      _screen->len_y = info.mod_y_resolution;
      ...
      _screen->buf.line
        = (uint8_t **)malloc(_screen->len_y * sizeof(uint8_t *));

      for (line = 0; _screen->len_y > line; line++)
        {
          _screen->buf.line[line] = (uint8_t *)(_screen->stride * line);
        }

      _screen->clip.pos_x1 = 0;
      _screen->clip.pos_y1 = 0;
      _screen->clip.pos_x2 = (info.mod_x_resolution - 1);
      _screen->clip.pos_y2 = (info.mod_y_resolution - 1);
    }
  while (0);
  ...
  display_auto_set_mode()
{
  static struct
  {
    size_t len_x;
    size_t len_y;
  } res[] = { { 1400, 1050 }, { 1024, 768 }, { 800, 600 }, { 640, 480 } };
  int mode;
  int rc = -1;
  size_t slot;
  ...
  for (slot = 0; (sizeof(res) / sizeof(res[0])) > slot; slot++)
    {
      mode = vbe_query_mode(res[slot].len_x, res[slot].len_y, 32);
      if (-1 != mode)
        {
