...
#  pragma pack(pop)
  ...
  extern int
    vbe_get_info_block(struct vbe_info_block *);

  extern void
    vbe_cleanup();

  extern int
    vbe_get_mode_block(struct vbe_mode_block *const, int16_t const);

  extern int
    vbe_query_installed();

  extern int
    vbe_query_mode(unsigned const, unsigned const, unsigned const);

  extern int16_t
    vbe_get_mode();

  extern int
    vbe_set_mode(unsigned const);
