...
void Fl_Preferences::enumerate(enum_cb cb, void *user_data) const // ALLEGRO:
{
  if (node)
  {
    node->enumerate(cb, user_data);
  }
  return;
}

void Fl_Preferences::Node::enumerate(Fl_Preferences::enum_cb cb, void* user_data) const // ALLEGRO:
{

  for (int i = 0; i < nEntry_; i++)
  {
    Fl_Preferences::Entry* entry= &entry_[i];
    bool cont= (*cb)(user_data, path_, entry->name, entry->value, i);
    if (false == cont)
    {
      break;
    }
  ...
  if (next_)
  {
    next_->enumerate(cb, user_data);
  }

  if (child_)
  {
    child_->enumerate(cb, user_data);
  }
...
: prefs_(prefs),
  ...
  if (!application) {
    ...
    snprintf(filename, sizeof(filename), "%s/%s.%s", path, application, Fl::system_driver()->preference_ext()); // ALLEGRO:
